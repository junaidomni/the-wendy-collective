...
const mocks = vi.hoisted(() => ({ advanceAdvisorDealWorkflow: vi.fn(), advanceGroupWorkflow: vi.fn(), createAdvisorAlert: vi.fn(), createAdvisorAvailabilityBlock: vi.fn(), createAdvisorDeal: vi.fn(), createAdvisorResearchOption: vi.fn(), createClientProposal: vi.fn(), createProposalResponse: vi.fn(), createTravelerProfileLink: vi.fn(), createTravelerProfileResponse: vi.fn(), deleteAdvisorAvailabilityBlock: vi.fn(), deletePublicWebsiteInquiry: vi.fn(), ensureGrimsleyCruiseExperience: vi.fn(), ensureGrimsleyGroupProfile: vi.fn(), getGroupTravelProfile: vi.fn(), getPrivateClientProposal: vi.fn(), getPrivateGroupTravelProfile: vi.fn(), getPrivateTravelerProfile: vi.fn(), listAdvisorAlerts: vi.fn(), listAdvisorAppointments: vi.fn(), listAdvisorAvailabilityBlocks: vi.fn(), listAdvisorDeals: vi.fn(), listAdvisorResearchOptions: vi.fn(), listClientProposals: vi.fn(), listCruiseExperiences: vi.fn(), listProposalResponses: vi.fn(), listTravelerProfileLinks: vi.fn(), listTravelerProfileResponses: vi.fn(), listWorkflowStageEvents: vi.fn(), markAdvisorAlertRead: vi.fn(), markClientProposalShared: vi.fn(), reopenAdvisorDealWorkflow: vi.fn(), reopenGroupWorkflow: vi.fn(), saveGroupWorkflowDetails: vi.fn(), scheduleLocalDiscoveryAppointment: vi.fn(), shareGroupTravelProfile: vi.fn(), syncExistingRequestsToAdvisorDeals: vi.fn(), updateAdvisorDeal: vi.fn(), updateGroupTravelProfile: vi.fn(), updateProposalResponseStatus: vi.fn(), createCruiseExperience: vi.fn(), notifyOwner: vi.fn() }));
...
describe("advisor CRM and private proposal workflow", () => {
  beforeEach(() => {
    ...
    mocks.listAdvisorAlerts.mockResolvedValue([]); mocks.listAdvisorAppointments.mockResolvedValue([]); mocks.listAdvisorAvailabilityBlocks.mockResolvedValue([]); mocks.listAdvisorDeals.mockResolvedValue([]); mocks.listAdvisorResearchOptions.mockResolvedValue([]); mocks.listClientProposals.mockResolvedValue([]); mocks.listCruiseExperiences.mockResolvedValue([]); mocks.listProposalResponses.mockResolvedValue([]); mocks.listTravelerProfileLinks.mockResolvedValue([]); mocks.listTravelerProfileResponses.mockResolvedValue([]); mocks.advanceAdvisorDealWorkflow.mockResolvedValue({ stage: "discovery_call" }); mocks.advanceGroupWorkflow.mockResolvedValue({ workflowStage: "proposal_shared" }); mocks.listWorkflowStageEvents.mockResolvedValue([]); mocks.createAdvisorAlert.mockResolvedValue({ id: 92 }); mocks.markAdvisorAlertRead.mockResolvedValue(undefined); mocks.scheduleLocalDiscoveryAppointment.mockResolvedValue({ id: 151 }); mocks.createAdvisorAvailabilityBlock.mockResolvedValue({ id: 41 }); mocks.deleteAdvisorAvailabilityBlock.mockResolvedValue(undefined); mocks.createTravelerProfileLink.mockResolvedValue({ id: 117 }); mocks.createAdvisorResearchOption.mockResolvedValue({ id: 118 }); mocks.createTravelerProfileResponse.mockResolvedValue({ id: 119 });
  ...
  it("keeps normal-client profile links and research options private to Wendy", async () => {
    const caller = appRouter.createCaller(context(owner));
    const link = await caller.crm.createTravelerProfileLink({ dealId: 51, validForDays: 60 });
    expect(link.privateToken).toMatch(/^[A-Za-z0-9_-]{40,}$/);
    expect(mocks.createTravelerProfileLink).toHaveBeenCalledWith(expect.objectContaining({ dealId: 51, privateToken: link.privateToken, expiresAt: expect.any(Date) }));
    await caller.crm.createResearchOption({ dealId: 51, optionType: "cruise", title: "A considered Caribbean sailing", sourceReference: "https://example.com/advisor-reference", reviewedOn: "2026-08-31", clientFit: "Matches a relaxed family celebration." });
    expect(mocks.createAdvisorResearchOption).toHaveBeenCalledWith(expect.objectContaining({ dealId: 51, optionType: "cruise", title: "A considered Caribbean sailing" }));
    await expect(appRouter.createCaller(context()).crm.createTravelerProfileLink({ dealId: 51 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(appRouter.createCaller(context()).crm.createResearchOption({ dealId: 51, optionType: "cruise", title: "A considered Caribbean sailing" })).rejects.toMatchObject({ code: "FORBIDDEN" });
  });

  it("stores repeat traveler-profile responses on their originating client link without a new pipeline record", async () => {
    mocks.getPrivateTravelerProfile.mockResolvedValue({ link: { id: 117, dealId: 51, status: "shared" }, deal: { id: 51, contactFirstName: "Alex", contactLastName: "Morgan" } });
    const result = await appRouter.createCaller(context()).crm.submitTravelerProfile({ token, contactFirstName: "Alex", contactLastName: "Morgan", email: "alex@example.com", phone: "555-010-1020", travelerDetails: [{ fullName: "Alex Morgan", dateOfBirth: "1985-03-10", loyaltyNumber: "" }], preferences: { destinationIdeas: "Caribbean", travelWindow: "Summer 2027", departurePort: "Atlanta", budgetDirection: "Flexible", roomPreferences: "Balcony", paceAndStyle: "Relaxed", accessibilityNeeds: "", mustHaves: "Beach time" }, notes: "Celebrating an anniversary.", consent: true });
    expect(result).toMatchObject({ success: true, responseId: 119, unavailable: false });
    expect(mocks.createTravelerProfileResponse).toHaveBeenCalledWith(expect.objectContaining({ travelerProfileLinkId: 117, travelerDetailsJson: expect.stringContaining("Alex Morgan") }));
    expect(mocks.createAdvisorAlert).toHaveBeenCalledWith(expect.objectContaining({ sourceType: "traveler_profile", sourceId: 119, href: "/wendy/clients/51" }));
    expect(mocks.createAdvisorDeal).not.toHaveBeenCalled();
    mocks.getPrivateTravelerProfile.mockResolvedValue(undefined);
    await expect(appRouter.createCaller(context()).crm.submitTravelerProfile({ token, contactFirstName: "Alex", contactLastName: "Morgan", email: "alex@example.com", phone: "555-010-1020", travelerDetails: [{ fullName: "Alex Morgan", dateOfBirth: "", loyaltyNumber: "" }], preferences: { destinationIdeas: "", travelWindow: "", departurePort: "", budgetDirection: "", roomPreferences: "", paceAndStyle: "", accessibilityNeeds: "", mustHaves: "" }, consent: true })).resolves.toEqual({ success: false, unavailable: true });
  });
});
