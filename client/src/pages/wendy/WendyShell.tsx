...
export default function WendyShell({ active, title, eyebrow, action, children }: { active: string; title: string; eyebrow: string; action?: ReactNode; children: ReactNode }) {
  ...
  if (!user) return <main className="wendy-workspace"><div className="wendy-login-card"><p className="eyebrow">The Wendy Collective</p><h1 className="display display--small">Staff <em>login.</em></h1><p>Sign in to manage conversations, proposals, and travel plans.</p><Link href="/wendy/login" className="button-submit">Open staff login <span aria-hidden="true">↗</span></Link></div></main>;
  if (!isOwner) return <main className="wendy-workspace"><div className="wendy-login-card"><p className="eyebrow">The Wendy Collective</p><h1 className="display display--small">This space is reserved for <em>staff.</em></h1><p>Please return to the main site to begin your travel conversation.</p></div></main>;
  return <main className="crm-app"><aside className="crm-sidebar"><Link href="/wendy" className="crm-brand"><span>TWC</span><strong>The Wendy Collective</strong></Link><nav aria-label="Wendy workspace navigation">{navigation.map((item) => <Link key={item.href} href={item.href} className={active === item.href ? "is-active" : ""}>{item.label}</Link>)}</nav><div className="crm-sidebar__footer"><span>Advisor workspace</span><strong>{user.name || "Wendy"}</strong><button type="button" className="crm-logout" onClick={async () => { await logout(); setLocation("/wendy/login", { replace: true }); }}>Sign out</button></div></aside><section className="crm-main"><header className="crm-page-header"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1></div>{action}</header>{children}</section></main>;
}
